...
- `client_research_data.tavilySearchUsed` (boolean, default false) — true when the Tavily LlmAgent returned non-empty prose (`tavilyIntel.length > 0`); query this in Neon to confirm Tavily web search fired
...
- `clientName` — step-2 (`runRfpParser`) returns `{ clientName }` via Inngest cached step result; passed directly as `parserResult?.clientName ?? null` to step-3. DB read in Agent 3 is a fallback only. **Use `|| null` not `?? null`** when processing `clientName` from Gemini output — Gemini sometimes returns `""` (empty string) which `?? null` passes through unchanged, causing the LLM research agent to search for a blank company name.
...
// return await so Inngest captures the result for dashboard display
...
**Inngest step output / dashboard visibility:**
...
// ❌ — Inngest shows null; no debugging signal
await step.run('step-3-client-research', async () => {
  await runClientResearch({ ... })  // missing return
})
```
...
`tavilySearchUsed` is set `true` when the LlmAgent pass returns non-empty prose (`tavilyIntel.length > 0`). `sources[]` is populated separately by the synthesis LLM extracting URLs from that prose — `sources` can be empty even when Tavily fired (synthesis LLM may not extract URLs reliably). Both passes are non-fatal — failure falls back to LLM-knowledge-only profile.
...
`src/app/favicon.ico` has been deleted. The icon is set via `metadata.icons` in `src/app/layout.tsx` pointing to a Cloudinary PNG URL. Do not re-add a `favicon.ico` file — it would take precedence over the metadata and revert the icon.

### Key Pages

- `/` (landing page) — server component (`src/app/page.tsx`). Renders `<IntroSplashWrapper />` first (a thin `'use client'` component at `src/components/landing/IntroSplashWrapper.tsx` that wraps `IntroSplash` via `next/dynamic` with `ssr: false` — required because `page.tsx` is a server component and `ssr: false` is not allowed there directly). `IntroSplash` shows only **once per browser** — on first visit it writes `localStorage.setItem('nivedanai_intro_seen', '1')` on dismiss; subsequent visits skip it entirely via lazy `useState(() => !localStorage.getItem('nivedanai_intro_seen'))`. The overlay shows an infographic image with zoom/pan controls. Dismiss via "Enter site" button, "Enter Nivedan AI" CTA, ESC key, or clicking outside the frame. Zoom: scroll wheel (non-passive native listener), `+`/`−` buttons, keyboard `+`/`-`/`0`, drag-to-pan, double-click to reset. Styles in `IntroSplash.module.css` — the `introImgIn` animation lives on `.frame` (wrapper); zoom `transform` is on `.frameImg` (inner `<img>` with no CSS animation) to avoid the fill-mode override trap. Zoom toolbar is positioned to the right of the frame (row layout). An ambient robot video (`.introVideo`) sits to the left of the frame — `autoPlay muted loop playsInline`, `mix-blend-mode: screen` + radial `mask-image` to blend the dark space background seamlessly, `pointer-events: none` so click-outside-dismiss still works. Hidden at ≤900 px.
**"See How It Works" CTA (hero section, `src/components/landing/Hero.tsx`):** Unauthenticated click → opens `VideoModal` (defined inline in `Hero.tsx`) — full-screen backdrop with custom controls (seek slider, ±10s skip, play/pause, fullscreen). Video URL is `VIDEO_URL` constant at top of `VideoModal`. Authenticated click → `router.push('/redirecting?to=how-it-works')`. Controls bar has `onClick={e.stopPropagation()}` and all buttons have `type="button"` — required to prevent clicks reaching the `<video>` element below. The `<video>` element itself has NO `onClick`.
...
- `/workflow/[jobId]` — polls `/api/jobs/[jobId]` every 3s. `CircularProgress` shows ETA while running, "Complete" + "View Proposal" button when `awaiting_review`/`completed`. During the initial pending delay (before Inngest fires the first agent), a `STARTUP_MESSAGES` array rotates through 9 messages every 2.8s (`msgIdx` state + `useEffect` that clears when `job.status !== 'pending'`) — purely cosmetic, no logic change. **HITL "Request Changes":** `POST /api/proposals/[jobId]/changes` must include ALL rewritable sections in `flaggedSections` — passing `[]` causes `handle-hitl-changes.ts` to exit early and no sections get rewritten (only Quality Review re-runs). Valid sections: `executiveSummary`, `understandingOfRequirements`, `proposedSolution`, `technicalApproach`, `caseStudies`, `teamAndExpertise`, `projectTimeline`, `pricingStructure`.
